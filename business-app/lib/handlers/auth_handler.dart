...
class AuthHandler {
  _getInitialRoute(BuildContext context) async {
    ...
    if (authTOken != null) {
      // bool isDone = await auth_service.googleAuth(context: context);
      var body = json.decode(authTOken);
      Provider.of<UserProvider>(context, listen: false).setUser(User.fromJson(body));
      bool isDone = true;
      if (isDone) {
        Timer(const Duration(seconds: 2),
            () => Navigator.of(context).pushNamedAndRemoveUntil(MyRoute.dashboard, (Route<dynamic> route) => false));
      } else {
        Timer(const Duration(seconds: 2), () => Navigator.of(context).pushReplacementNamed(MyRoute.loginPage));
      }
    } else {
      showToast('Please Login');
      Timer(const Duration(seconds: 2),
          () => Navigator.of(context).pushNamedAndRemoveUntil(MyRoute.welcome, (Route<dynamic> route) => false));
    }
  }
